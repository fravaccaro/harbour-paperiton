...
Page {
    ...
    readonly property string readScript:
        ...
        + "    request.open('GET', '/api/profile/', false);"
        + "    request.setRequestHeader('Accept', 'application/json');"
        + "    request.send();"
        + "    if (request.status !== 200) return 'failed:' + request.status;"
        + "    var token = JSON.parse(request.responseText).auth_token;"
    ...
    readonly property string createScript:
        "return (function() {"
        + "  try {"
    ...
    function tokenReceived(result) {
        ...
        page.errorMessage = qsTr("The server refused to create a token (%1).")
                                .arg(answer.replace("failed:", ""))
    }
    ...
    Column {
        ...
        Label {
            ...
            text: {
                ...
                return qsTr("Sign in as you would in a browser. Paperiton then takes the API token "
                            + "of your account from the server.")
            }
        }
