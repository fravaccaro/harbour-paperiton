...
Dialog {
    ...
    canAccept: addTagIds.length > 0 || removeTagIds.length > 0 || correspondentId > 0 || documentTypeId > 0
    ...
    SilicaFlickable {
        ...
        Column {
            ...
            DialogHeader {
                title: dialog.documentCount === 1 ? qsTr("Edit one document") : qsTr("Edit %1 documents").arg(dialog.documentCount)
                acceptText: qsTr("Apply")
            }

            ValueButton {
                label: qsTr("Add tags")
                value: dialog.addTagIds.length > 0 && Tags.ready ? Tags.namesFor(dialog.addTagIds).join(", ") : qsTr("None")
                onClicked: {
            ...
            ValueButton {
                ...
                value: dialog.removeTagIds.length > 0 && Tags.ready ? Tags.namesFor(dialog.removeTagIds).join(", ") : qsTr("None")
                onClicked: {
                    var picker = pageStack.push(Qt.resolvedUrl("LookupPickerPage.qml"), {
                        "lookup": Tags,
            ...
            ValueButton {
                ...
                value: dialog.correspondentId > 0 && Correspondents.ready ? Correspondents.nameFor(dialog.correspondentId) : qsTr("Unchanged")
                onClicked: {
                    var picker = pageStack.push(Qt.resolvedUrl("LookupPickerPage.qml"), {
            ...
            ValueButton {
                label: qsTr("Type")
                value: dialog.documentTypeId > 0 && DocumentTypes.ready ? DocumentTypes.nameFor(dialog.documentTypeId) : qsTr("Unchanged")
                onClicked: {
                    var picker = pageStack.push(Qt.resolvedUrl("LookupPickerPage.qml"), {
