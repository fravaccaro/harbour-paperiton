...
Dialog {
    ...
    function customFieldsPatch() {
        ...
        for (var key in customValues) {
            ...
            if (value === "" || value === undefined || value === null)
                entries.push({ "field": fieldId, "value": null })
            else if (dataType === "integer")
                entries.push({ "field": fieldId, "value": parseInt(value, 10) })
            else if (dataType === "float")
                entries.push({ "field": fieldId, "value": parseFloat(value) })
            else if (dataType === "boolean")
                entries.push({ "field": fieldId, "value": value === true || value === "true" })
            else
                entries.push({ "field": fieldId, "value": String(value) })
        }
        return entries
    }
    ...
    Component.onCompleted: {
        ...
        for (var i = 0; i < existing.length; ++i)
            values[existing[i].field] = existing[i].value !== null ? existing[i].value : ""
        customValues = values
        ...
        Column {
            ...
            ValueButton {
                ...
                value: dialog.correspondentId > 0 && Correspondents.ready
                       ? Correspondents.nameFor(dialog.correspondentId) : qsTr("None")
                onClicked: {
            ...
            ValueButton {
                ...
                value: dialog.documentTypeId > 0 && DocumentTypes.ready
                       ? DocumentTypes.nameFor(dialog.documentTypeId) : qsTr("None")
                onClicked: {
                    var picker = pageStack.push(Qt.resolvedUrl("LookupPickerPage.qml"), {
            ...
            ValueButton {
                label: qsTr("Tags")
                value: dialog.tagIds.length > 0 && Tags.ready ? Tags.namesFor(dialog.tagIds).join(", ")
                                                              : qsTr("None")
                onClicked: {
                    var picker = pageStack.push(Qt.resolvedUrl("LookupPickerPage.qml"), {
                                                    lookup: Tags,
            ...
            ValueButton {
                label: qsTr("Created")
                value: dialog.created && !isNaN(dialog.created.getTime())
                       ? Qt.formatDate(dialog.created, Qt.DefaultLocaleShortDate) : qsTr("Unknown")
                onClicked: {
                    var picker = pageStack.push("Sailfish.Silica.DatePickerDialog", {
                                                    date: dialog.created && !isNaN(dialog.created.getTime())
            ...
            Repeater {
                ...
                Column {
                    ...
                    TextField {
                        ...
                        inputMethodHints: model.dataType === "integer" || model.dataType === "float"
